...
Similiarly using gap analyzer and running the same on bigger vs smaller model, I noticed the drop in accuracy was manageable therefore used the smaller model.. when run with bigger model same dataset could have best F1 at 0.70 at same F1 max score
